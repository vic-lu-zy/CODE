function [h,ax,BigAx,patches,pax] = myplotmatrix(varargin)
...
elseif nin==2, % plotmatrix(x,y)
    rows = size(args{1},2); cols = size(args{1},2);
    x = args{1}; y = args{2};
    dohist = 1;
else
...
if any(sym=='.'),
    ...
    markersize = max(1,min(4,round(15*min(pos(3:4))/max(1,size(x,1))/max(rows,cols))));
else
...
for i=rows:-1:1,
    for j=cols:-1:1,
        ...
        hh(i,j,:) = gscatter(x(:,i),x(:,j),y==1,'rb','ox');
        legend off
...
if dohist, % Put a histogram on the diagonal for plotmatrix(y) case
    ...
    for i=rows:-1:1,
        ...
        %         [nn,xx] = hist(reshape(x(:,i,:),[m k]));
        %         patches(i,:) = bar(histax,xx,nn,'hist');
        boxplot(histax,x(:,i),y);
        set(histax,'xtick',[],'ytick',[],'xgrid','off','ygrid','off');
        set(histax,'ylim',[ylimmin(i,1) ylimmax(i,1)])
